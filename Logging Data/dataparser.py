...
def show_heat_map(qid, action_data, game_over_data, damage_data):
	...
	for (x, y) in heat[qid]:
		...
		yCor.append(y)
	# plot the heat map
	ax1.scatter(xCor, yCor, c='y', alpha=0.05)

	xCor = []
	yCor = []
	for d in game_over_data:
		if d[0] == qid:
			xCor.append(d[1])
			yCor.append(d[2])
	# plot the game over map
	ax1.scatter(xCor, yCor, c='b', alpha=0.3)
	...
	for d in damage_data:
		if d[0] == qid:
			...
			yCor.append(d[2])
	# plot the damage map
	ax1.scatter(xCor, yCor, c='r', alpha=0.05)
